...
@dataclass
class MSG_COAST(BlockProcessor):
    ...
    tir1: Any
    tir2: Any
    name: str = field(default='MSG_cloudmask', init=False)
        
    def input_vars(self):
        return [names.rtoa, names.bt]
    
    def created_vars(self):
         return [Var(self.name, dtype='uint8', dims_like=0)]
    
    def process_block(self, block):

        water = block[self.water]
        ...
        block[self.name] = where(water, sea_mask, land_mask)
        block[self.name].attrs.update(label)  # Add label definitions to mask attributes